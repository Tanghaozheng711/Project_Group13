...
For the clustering analysis, latent clustering was used. 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, out.width='80%', fig.cap='Confusion Matrix', fig.pos = 'h'}
knitr::include_graphics("decisiontree2.png")
```

# Findings

Through various combinations of variable selection, we found that the top 4 variables that are important in predicting a student's grades are number of failures, number of absences, weekend alcohol consumption as well as family relationship.

With number of failures, it will definitely be a good predictor since it will show the competency level of the student in being able to do well. Absences may show the attitude the student has on studying, or if there are legitimate reasons for being absent, missing out on class would have an impact on the student's learning. 

For alcohol consumption and family relationship, studies have been done to show how these factors do affect one's academic performance to a certain extent. For example, in papers by Mills et al. (2013) and Shahrier et al. (2015) showing the impact that alcohol consumption and family relationships have on academic performance respectively. 
...
- id: Shahrier1
  title: The relationship between academic performance and family relation of secondary school students
  author:
  - family: Shahrier
    given: M A
  - family: Islam
  ...
  - family: Khatun
    given: Mahbuba
  URL: 'https://www.researchgate.net/publication/327679318_The_relationship_between_academic_performance_and_family_relation_of_secondary_school_students'
  type: article-journal
  issued:
    year: 2015
    month: 01
